...
class ApodRecyclerAdapter(private val vm: ApodViewModel) : androidx.recyclerview.widget.ListAdapter<ImageOfTheDayModel, ApodViewHolder>(
    diffUtil) {
...
class ApodViewHolder(private val binding: ApodItemBinding, private val vm: ApodViewModel) : RecyclerView.ViewHolder(
    ...
    companion object {
        fun create(parent: ViewGroup, vm: ApodViewModel) : ApodViewHolder{
            val view = LayoutInflater.from(parent.context).inflate(R.layout.apod_item,
                parent,
                false)
